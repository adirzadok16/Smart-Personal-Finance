...
transactionRoutes.post('/transaction', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Transaction added successfully", success: true, transaction: transaction.id })
  } catch (error) {
    console.error(error);
...
transactionRoutes.delete('/transaction', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Transaction deleted successfully", success: true, transaction: transaction.id })
  } catch (error) {
    console.error(error);
...
transactionRoutes.put('/transaction', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Transaction updated successfully", success: true, transaction: transaction.id })
  } catch (error) {
    console.error(error);
